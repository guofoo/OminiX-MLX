...
/// Resample audio using high-quality sinc interpolation (rubato)
///
/// This uses a windowed sinc interpolation algorithm which is mathematically
/// optimal for bandlimited signals. Much higher quality than linear interpolation,
/// especially important for speech/audio processing pipelines like HuBERT.
///
/// # Arguments
...
pub fn resample(samples: &[f32], src_rate: u32, target_rate: u32) -> Vec<f32> {
    ...
    // High-quality sinc interpolation parameters (matching librosa quality)
    let params = SincInterpolationParameters {
        sinc_len: 256,                              // Long kernel for high quality
        f_cutoff: 0.95,                             // Cutoff frequency ratio
        interpolation: SincInterpolationType::Cubic, // Cubic interpolation (highest quality)
        oversampling_factor: 256,                   // High oversampling for precision
    ...
    // Process in chunks to handle arbitrary input length
    ...
    while pos + chunk_size <= samples.len() {
        ...
        match resampler.process(&chunk, None) {
            ...
            Err(e) => {
                eprintln!("Warning: Sinc resampling failed at chunk {}: {}. Falling back to linear.", pos, e);
                return resample_linear(samples, src_rate, target_rate);
            }
        }
        pos += chunk_size;
    ...
    // Process remaining samples (partial chunk) - important for matching librosa output length
    if pos < samples.len() {
        let remaining = samples.len() - pos;
        // Pad with zeros to make a full chunk, then process as partial
        let mut padded = vec![0.0f32; chunk_size];
        padded[..remaining].copy_from_slice(&samples[pos..]);

        // Process the partial input
        let chunk = vec![padded];
        match resampler.process_partial(Some(&chunk), None) {
            Ok(out) => {
                if !out.is_empty() {
                    // Only take the samples corresponding to the actual input
            ...
            Err(e) => {
                eprintln!("Warning: Sinc partial resampling failed: {}.", e);
            }
        }
    }
...
pub fn save_wav(samples: &[f32], sample_rate: u32, path: impl AsRef<Path>) -> Result<(), std::io::Error> {
    use std::io::Write;
...
/// Fallback linear interpolation resampler (lower quality but always works)
...
/// Load audio for HuBERT (16kHz)
///
/// Returns Array with shape [1, samples] ready for HuBERT input.
/// Note: HuBERT encoder applies its own normalization (zero-mean, unit-variance)
/// matching Wav2Vec2FeatureExtractor, so we don't normalize here.
pub fn load_audio_for_hubert(
    ...
    // Note: No max-normalization here. HuBERT encoder does proper normalization
    // (zero-mean, unit-variance) matching Wav2Vec2FeatureExtractor.
