//! Text generation example with GLM-4.7-Flash (MoE + MLA)

use std::env;
use std::time::Instant;
use mlx_rs::ops::indexing::{IndexOp, NewAxis};
use mlx_rs::transforms::eval;
use glm47_flash_mlx::{load_model, load_tokenizer, Generate, KVCache, Error};
...
fn main() -> Result<(), Error> {
    ...
    if args.len() < 2 {
        ...
        eprintln!("  huggingface-cli download inferencerlabs/GLM-4.7-Flash-MLX-6.5bit \\");
        eprintln!("      --local-dir ./models/GLM-4.7-Flash-MLX-6.5bit");
        std::process::exit(1);
    }
    ...
    let prompt = args.get(2)
        .map(|s| s.as_str())
        .unwrap_or("Hello, tell me about yourself.");

    println!("Loading GLM-4.7-Flash model from: {}", model_dir);
